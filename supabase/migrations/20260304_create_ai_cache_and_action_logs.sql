...
CREATE POLICY "Enable insert for everyone" ON public.ai_cache FOR INSERT WITH CHECK (true);

DROP POLICY IF EXISTS "Enable update for everyone" ON public.ai_cache;
CREATE POLICY "Enable update for everyone" ON public.ai_cache FOR UPDATE USING (true);

-- action_logs Policies
DROP POLICY IF EXISTS "Enable insert for everyone" ON public.action_logs;
CREATE POLICY "Enable insert for everyone" ON public.action_logs FOR INSERT WITH CHECK (true);
