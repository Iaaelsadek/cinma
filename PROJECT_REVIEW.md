...
- **تحسين الأداء (Performance Optimization):**
  - إصلاح مشاكل إزاحة التخطيط (CLS) بنسبة كبيرة عن طريق تحديد أبعاد الصور مسبقاً (Aspect Ratio) وضبط الهياكل العظمية (Skeletons).
  - تحسين تحميل الخطوط باستخدام `font-display: swap`.
  - تطبيق التحميل الكسول (Lazy Loading) لجميع الصور خارج إطار العرض، والتحميل المسبق (Eager Loading) لصور الـ Hero.
- **إصلاح المحتوى (Content Fixes):**
  - تصحيح استعلامات قاعدة البيانات لأقسام المسرحيات ورمضان.
  - إضافة ميزة "تخطي المقدمة" (Skip Intro) في المشغل.
- **نظام الترشيحات (Recommendation Engine):**
  - بناء نظام ذكي يحلل سجل المشاهدة ويقترح محتوى مشابه بناءً على التصنيف والممثلين.
