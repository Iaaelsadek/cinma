/**
 * 11 free streaming servers extracted from cinma.online project.
 * Builds embed URLs for movies and TV shows using TMDB IDs.
 */
...
/** Append a query param only if not already present */
...
/** Add Arabic subtitle/lang params per server — applies to ALL servers */
const withArabic = (url: string, serverId: string, _mediaType: 'movie' | 'tv'): string => {
  const id = serverId.toLowerCase();
  // كل السيرفرات تحصل على Arabic params
  ...
  if (id.startsWith('2embed')) return appendParam(appendParam(url, 'lang', 'ar'), 'subtitles', 'ar');
  if (id === 'smashystream') return appendParam(appendParam(url, 'lang', 'ar'), 'sub', 'ar');
  if (id === '111movies') return appendParam(url, 'lang', 'ar');
  return appendParam(url, 'lang', 'ar');
};

/** Build embed URL for a given server + TMDB ID */
export const buildServerUrl = (
  ...
  if (id.startsWith('2embed')) {
    url = mediaType === 'movie' ? `${base}/${tmdbId}` : `${base}/${tmdbId}/${season}/${episode}`;
    return withArabic(url, id, mediaType);
  }
...
/** Build all 11 server sources for a content item */
export const buildAllServerSources = (
  tmdbId: number,
  mediaType: 'movie' | 'tv',
  season = 1,
  episode = 1
): Array<{ name: string; url: string; quality: string }> => {
