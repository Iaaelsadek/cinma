...
const fetchRamadanSeries = async (year: number) => {
  ...
  if (error) {
    console.error('Error fetching Ramadan series:', error)
    return []
  }
...
const fetchClassicRamadan = async () => {
    ...
    if (error) {
      console.error('Error fetching classic Ramadan series:', error)
      return []
    }
    return data.map((item: any) => ({ ...item, media_type: 'tv' }))
}
    ...
    <div className="min-h-screen text-white pb-24 max-w-[2400px] mx-auto px-4 md:px-12 w-full">
      ...
      <div className="space-y-8 -mt-20 relative z-10 px-4 md:px-0">
        ...
        <div className="flex items-center gap-3 mb-8 border-b border-amber-500/20 pb-6">
             <div className="p-3 bg-amber-500/10 rounded-full border border-amber-500/20 shadow-[0_0_15px_rgba(245,158,11,0.2)]">
                <Moon size={32} className="text-amber-400 fill-amber-400 drop-shadow-[0_0_10px_rgba(245,158,11,0.5)]" />
             </div>
             <div>
                <h1 className="text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-amber-200 via-amber-400 to-amber-600">
                  {lang === 'ar' ? 'الخيمة الرمضانية' : 'Ramadan Tent'}
                </h1>
                <p className="text-amber-200/60 text-lg mt-1">
