...
const routeMap: Array<{ match: RegExp | string; load: () => Promise<unknown> }> = [
  ...
  { match: /^\/movie\/[a-z0-9-]+$/, load: () => import('../pages/media/MovieDetails') },
  ...
  { match: /^\/series\/[a-z0-9-]+$/, load: () => import('../pages/media/SeriesDetails') },
  { match: /^\/series\/\d+$/, load: () => import('../pages/media/SeriesDetails') },
  { match: /^\/series\/[a-z0-9-]+\/season/, load: () => import('../pages/media/Watch') },
